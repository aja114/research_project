...
for a in algos:
	data[a] = pd.read_csv(f'{exp}/{a}.csv', header=[0, 1], skipinitialspace=True)
...
# pos = range(-len(algos)/2, len(algos)/2, 0.5)
pos = [-1.5, -0.5, 0.5, 1.5]
print(pos)
for i, a in enumerate(algos):
	...
	rect = ax.bar(x + pos[i] * width, mean, width*0.8, yerr=error, ecolor='black', label=a)
